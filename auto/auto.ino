...
#include "mouving_average.h"                      // simple moving average class; for Sonar functionality
...
// LCD Display
//LiquidCrystal_I2C (0x3F, 20, 4);  // Set the LCD I2C address and size (4x20)
#define LEFT_ARROW 0x7F
#define RIGHT_ARROW 0x7E
#define DEGREE_SYMBOL 0xDF
//char _buffer[20];      
//PString message(_buffer, sizeof(_buffer));    // holds message we display on line 4 of LCD
...
void setup() 
{
  ...
  //
  // Start LCD display
//  .begin();            // start the LCD...new version doesn't require size startup parameters
...
//      .setCursor(0, 1);
//      .print(F("Wait Time: "));
//      .print((int) (millis() - startTime) / 1000);     // show how long we have waited  
...
void updateDisplay(void)
{
  ...
  if (currentTime >= lastUpdate + 500 )   // limit refresh rate
  {
    ...
    if (headingError < 0)
      //.write(LEFT_ARROW);
    Serial.print(abs(headingError), DEC);
    if (headingError > 0)
      //.write(RIGHT_ARROW);
    Serial.print(F(" Dist "));
    Serial.print(distanceToTarget, DEC);
    Serial.print(F("m "));
    #ifdef USE_GRAPHING
      //.write(map(distanceToTarget, 0, originalDistanceToTarget, 0, 7));    // show tiny bar graph of distance remaining
    #endif
    ...
    #ifdef USE_GRAPHING
      //.write(map(sonarDistance, 0, MAX_DISTANCE_IN, 0, 7));
    #endif
    Serial.print(F(" Spd "));
    Serial.print(speed, DEC);
    #ifdef USE_GRAPHING
      //.write(map(speed, 0, 255, 0, 7));
    #endif
    ...
    Serial.print(F(" WPT "));
    Serial.print(waypointNumber + 1, DEC);
    Serial.print(F(" OF "));
    Serial.print(NUMBER_WAYPOINTS - 1, DEC);
    ...
    if(IR_receiver.decode(&IR_results))     // check for manual "kill switch"
        {
         ...
         // .clear(); 
          Serial.print(F("Press to resume"));
          delay(1000); 
